...
def ExplicitMatrixFactorisation(n_users, n_items, n_factors):
    ...
    item_embedding = Embedding(
        n_items, n_factors, embeddings_regularizer=l2(1e-6), name="ItemEmbedding"
    )(item_input)
    ...
    user_embedding = Embedding(
        n_users, n_factors, embeddings_regularizer=l2(1e-6), name="UserEmbedding"
    )(user_input)
...
def ExplicitMatrixFactorisationBias(
    n_users, n_items, n_factors, min_rating, max_rating
):
    ...
    # Item Bias
    item_bias = Embedding(n_items, 1, embeddings_regularizer=l2(1e-6), name="ItemBias")(
        item_input
    )
    ...
    # User Bias
    user_bias = Embedding(n_users, 1, embeddings_regularizer=l2(1e-6), name="UserBias")(
        user_input
    )
    ...
    # Model Creation
    model = Model(
        [user_input, item_input], rating_output, name="ExplicitMatrixFactorisationBias"
    )
...
def NeuralCollaborativeFiltering(n_users, n_items, n_factors, min_rating, max_rating):
    ...
    # Item Embedding MF
    item_embedding_mf = Embedding(
        n_items,
        n_factors,
        embeddings_regularizer=l2(1e-6),
        embeddings_initializer="he_normal",
        name="ItemEmbeddingMF",
    )(item_input)
    ...
    # Item Bias
    item_bias = Embedding(n_items, 1, embeddings_regularizer=l2(1e-5), name="ItemBias")(
        item_input
    )
    item_bias_vec = Flatten(name="FlattenItemBiasE")(item_bias)

    # User Bias
    user_bias = Embedding(n_users, 1, embeddings_regularizer=l2(1e-5), name="UserBias")(
        user_input
    )
    user_bias_vec = Flatten(name="FlattenUserBiasE")(user_bias)
